...
let historicalVolumeChartInstance = null;
let historicalStatusChartInstance = null;
const HISTORICAL_SHEET_SOURCE = {
    sheetId: '1a1M0fAAao7s28VsG4VlOfDf-6dvjlzZafPW_i2Qsz0M',
    months: [
        { key: '2025-12', label: 'Dec 2025', gid: '0' },
        { key: '2026-01', label: 'Jan 2026', gid: '1191472140' },
        { key: '2026-02', label: 'Feb 2026', gid: '606493654' },
        { key: '2026-03', label: 'Mar 2026', gid: '1938440491' }
    ]
};
...
function updateDashboard() {
    ...
    const popzoeEntries = entries.filter(e => e.platform === 'POPZOE');
    const popsurEntries = entries.filter(e => e.platform === 'POPSUR');
    const popbeaEntries = entries.filter(e => e.platform === 'POPBEA');
    const popluzWinners = winners.filter(w => w.platform === 'POPLUZ');
    const popn1Winners = winners.filter(w => w.platform === 'POPN1');
    const popzoeWinners = winners.filter(w => w.platform === 'POPZOE');
    const popsurWinners = winners.filter(w => w.platform === 'POPSUR');
    const popbeaWinners = winners.filter(w => w.platform === 'POPBEA');
    ...
    setText('popzoeEntries', popzoeEntries.length);
    setText('popsurEntries', popsurEntries.length);
    setText('popbeaEntries', popbeaEntries.length);
    ...
    setText('popzoeWinners', popzoeWinners.length);
    setText('popsurWinners', popsurWinners.length);
    setText('popbeaWinners', popbeaWinners.length);
...
function renderDailyAnomalyAlerts(entries) {
    ...
    const platforms = ['POPLUZ', 'POPN1', 'POPZOE', 'POPSUR', 'POPBEA'];
...
function historicalDetectDelimiter(headerLine) {
    const counts = {
...
function historicalParseCsvLine(line, delimiter = ',') {
    const values = [];
    let current = '';
    let inQuotes = false;
...
function historicalHeaderKey(raw) {
    return String(raw || '')
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toUpperCase()
...
function historicalNormalizeStatus(status) {
    const up = String(status || '').trim().toUpperCase();
    if (up === 'VALID' || up === 'VALIDADO' || up === 'VALIDATED' || up === 'VÁLIDO') return 'VALID';
    if (up === 'INVALID' || up === 'INVÁLIDO' || up === 'REJECTED' || up === 'CANCELLED') return 'INVALID';
    return 'PENDING';
}

function historicalDateKeyFromCells(dateCell, timeCell) {
    const dateRaw = String(dateCell || '').trim();
    const timeRaw = String(timeCell || '').trim();
    if (!dateRaw) return null;

    const withTime = `${dateRaw} ${timeRaw}`.trim();
    const d1 = DateUtils.parseWeekdayDate(withTime)
        || DateUtils.parseWeekdayDate(dateRaw)
        || DateUtils.parseDDMMYYYY(withTime)
        || DateUtils.parseDDMMYYYY(dateRaw)
        || DateUtils.parseISO(dateRaw);

    return d1 ? DateUtils.normalizeToYYYYMMDD(d1) : null;
}

function historicalParseCsv(csvText, monthConfig) {
    const lines = String(csvText || '').split(/\r?\n/).filter(Boolean);
    if (lines.length <= 1) return [];

    const delimiter = historicalDetectDelimiter(lines[0]);
    const headerCells = historicalParseCsvLine(lines[0], delimiter);
    const headerMap = {};
    headerCells.forEach((header, idx) => {
        headerMap[historicalHeaderKey(header)] = idx;
    });

    const idxPlatform = headerMap['PLATFORM'] ?? 0;
    const idxGameId = headerMap['GAME ID'] ?? 1;
    const idxStatus = headerMap['STATUS'] ?? 7;
    const idxDate = headerMap['DATE'] ?? 8;
    const idxTime = headerMap['TIME'] ?? 9;

    const rows = [];
    for (let i = 1; i < lines.length; i++) {
        const row = historicalParseCsvLine(lines[i], delimiter);
        if (!row.length) continue;

        const platform = String(row[idxPlatform] || '').trim().toUpperCase();
        const gameId = String(row[idxGameId] || '').trim();
        const status = historicalNormalizeStatus(row[idxStatus] || '');
        const dateKey = historicalDateKeyFromCells(row[idxDate], row[idxTime]) || `${monthConfig.key}-01`;

        if (!platform && !gameId) continue;
        rows.push({
...
async function fetchHistoricalMonthRows(monthConfig) {
    const url = `https://docs.google.com/spreadsheets/d/${HISTORICAL_SHEET_SOURCE.sheetId}/export?format=csv&gid=${monthConfig.gid}&t=${Date.now()}`;
    const response = await fetch(url, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Failed ${monthConfig.label}: HTTP ${response.status}`);
    const csvText = await response.text();
    if (csvText.trim().startsWith('<!DOCTYPE') || csvText.trim().startsWith('<html')) {
        throw new Error(`Failed ${monthConfig.label}: sheet is not publicly readable`);
    }
    return historicalParseCsv(csvText, monthConfig);
}

function setHistoricalLoading(show, message = 'Loading historical data...', progress = 0) {
    const wrap = document.getElementById('historicalLoadingState');
    const text = document.getElementById('historicalLoadingText');
    const bar = document.getElementById('historicalLoadingBar');
    if (!wrap) return;
...
function summarizeHistoricalRowsByMonth(rows) {
    const monthMap = {};
    HISTORICAL_SHEET_SOURCE.months.forEach(m => {
        monthMap[m.key] = {
            monthKey: m.key,
            month: m.label,
            total: 0,
            ...
            invalid: 0,
            pending: 0,
            uniqueGameIdsSet: new Set()
        };
    });

    rows.forEach(row => {
        if (!monthMap[row.monthKey]) return;
        const bucket = monthMap[row.monthKey];
        bucket.total++;
        if (row.gameId) bucket.uniqueGameIdsSet.add(row.gameId);
        if (row.platform === 'POPN1') bucket.popn1++;
        else if (row.platform === 'POPLUZ') bucket.popluz++;
        else if (row.platform === 'POPZOE') bucket.popzoe++;
        else if (row.platform === 'POPSUR') bucket.popsur++;
        else if (row.platform === 'POPBEA') bucket.popbea++;
        if (row.status === 'VALID') bucket.valid++;
        else if (row.status === 'INVALID') bucket.invalid++;
        else bucket.pending++;
    });

    return HISTORICAL_SHEET_SOURCE.months.map(m => {
        ...
        return {
            ...
            invalid: b.invalid,
            pending: b.pending,
            uniqueGameIds: b.uniqueGameIdsSet.size
        };
    });
}

function renderHistoricalSummary(rows, monthly) {
    ...
    rows.forEach(r => {
        total++;
        if (r.gameId) uniqueGlobal.add(r.gameId);
        if (r.platform === 'POPN1') popn1++;
        if (r.platform === 'POPLUZ') popluz++;
        if (r.platform === 'POPZOE') popzoe++;
        if (r.platform === 'POPSUR') popsur++;
        if (r.platform === 'POPBEA') popbea++;
        if (r.status === 'VALID') valid++;
        else if (r.status === 'INVALID') invalid++;
        else pending++;
    });

    setText('histTotalTickets', total);
    setText('histUniqueGameIds', uniqueGlobal.size);
    setText('histPopn1Tickets', popn1);
    setText('histPopluzTickets', popluz);
    setText('histPopzoeTickets', popzoe);
    setText('histPopsurTickets', popsur);
    setText('histPopbeaTickets', popbea);
    setText('histValidTickets', valid);
    setText('histInvalidTickets', invalid);
    setText('histPendingTickets', pending);
}

function renderHistoricalTable(monthly) {
    ...
    monthly.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td>${item.invalid}</td>
            <td>${item.pending}</td>
            <td>${item.uniqueGameIds}</td>
        `;
        tbody.appendChild(tr);
    });
}

function renderHistoricalTopGenerators(rows) {
    const tbody = document.getElementById('historicalTopGeneratorsBody');
    if (!tbody) return;
    tbody.innerHTML = '';
    ...
    rows.forEach(r => {
        ...
        map[key].total++;
        if (r.status === 'VALID') map[key].valid++;
        else if (r.status === 'INVALID') map[key].invalid++;
        else map[key].pending++;
    });

    const ranking = Object.values(map)
        .sort((a, b) => b.valid - a.valid || b.total - a.total || a.gameId.localeCompare(b.gameId))
        .slice(0, 50);

    if (!ranking.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="muted" style="text-align:center;">No data for this filter.</td></tr>';
        return;
    }
...
function renderHistoricalCharts(monthly) {
    const volumeCtx = document.getElementById('historicalVolumeChart')?.getContext('2d');
    const statusCtx = document.getElementById('historicalStatusChart')?.getContext('2d');
    if (!volumeCtx || !statusCtx) return;

    const labels = monthly.map(m => m.month);
    const totals = monthly.map(m => m.total);
    const valids = monthly.map(m => m.valid);
    const invalids = monthly.map(m => m.invalid);
    const pendings = monthly.map(m => m.pending);

    if (historicalVolumeChartInstance) historicalVolumeChartInstance.destroy();
    historicalVolumeChartInstance = new Chart(volumeCtx, {
        type: 'line',
        data: {
            labels,
            datasets: [{
    ...
    if (historicalStatusChartInstance) historicalStatusChartInstance.destroy();
    historicalStatusChartInstance = new Chart(statusCtx, {
        type: 'bar',
        data: {
            labels,
            datasets: [
                { label: 'VALID', data: valids, backgroundColor: '#22c55e' },
                { label: 'INVALID', data: invalids, backgroundColor: '#ef4444' },
...
function getHistoricalFilters() {
    return {
        month: document.getElementById('histMonthFilter')?.value || 'all',
        dateFrom: document.getElementById('histDateFromFilter')?.value || '',
        dateTo: document.getElementById('histDateToFilter')?.value || '',
        platform: (document.getElementById('histPlatformFilter')?.value || '').toUpperCase(),
        gameId: (document.getElementById('histGameIdFilter')?.value || '').trim()
    };
}

function applyHistoricalFilters() {
    const filters = getHistoricalFilters();
    let rows = historicalState.rows.slice();

    if (filters.month !== 'all') rows = rows.filter(r => r.monthKey === filters.month);
    if (filters.platform) rows = rows.filter(r => r.platform === filters.platform);
    if (filters.gameId) rows = rows.filter(r => String(r.gameId || '').includes(filters.gameId));
    ...
    historicalState.filteredRows = rows;
    const monthly = summarizeHistoricalRowsByMonth(rows);
    historicalState.monthly = monthly;
    renderHistoricalSummary(rows, monthly);
    renderHistoricalTable(monthly);
    renderHistoricalTopGenerators(rows);
    renderHistoricalCharts(monthly);
}

function setupHistoricalListeners() {
    if (historicalState.listenersAttached) return;
    document.getElementById('histApplyFiltersBtn')?.addEventListener('click', applyHistoricalFilters);
    document.getElementById('histClearFiltersBtn')?.addEventListener('click', () => {
        const month = document.getElementById('histMonthFilter');
        const from = document.getElementById('histDateFromFilter');
        const to = document.getElementById('histDateToFilter');
        const platform = document.getElementById('histPlatformFilter');
        const gameId = document.getElementById('histGameIdFilter');
    ...
    historicalState.listenersAttached = true;
}

async function initHistoricalPage(forceRefresh = false) {
    setupHistoricalListeners();
    if (historicalState.loading) return;
    if (historicalState.loaded && !forceRefresh) {
        applyHistoricalFilters();
        return;
    }

    historicalState.loading = true;
    hideError();
    setHistoricalLoading(true, 'Loading historical archive...', 0);
    try {
        const allRows = [];
        for (let i = 0; i < HISTORICAL_SHEET_SOURCE.months.length; i++) {
            const monthCfg = HISTORICAL_SHEET_SOURCE.months[i];
            setHistoricalLoading(true, `Loading ${monthCfg.label}...`, Math.round((i / HISTORICAL_SHEET_SOURCE.months.length) * 100));
            const rows = await fetchHistoricalMonthRows(monthCfg);
            for (let j = 0; j < rows.length; j++) {
                allRows.push(rows[j]);
            }
            setHistoricalLoading(true, `${monthCfg.label} loaded (${rows.length} rows).`, Math.round(((i + 1) / HISTORICAL_SHEET_SOURCE.months.length) * 100));
        }

        historicalState.rows = allRows;
        historicalState.loaded = true;
        applyHistoricalFilters();
    } catch (error) {
...
function displayWinnersList() {
    ...
    winnersState.filteredWinners.forEach(win => {
        ...
        const platform = String(win.platform || '').toUpperCase();
        const platformBadge = platform === 'POPLUZ'
            ? '<span class="badge" style="background:#9C27B0;color:white;">POPLUZ</span>'
            : platform === 'POPN1'
                ? '<span class="badge" style="background:#FF5722;color:white;">POPN1</span>'
                : platform === 'POPZOE'
                    ? '<span class="badge" style="background:#f59e0b;color:white;">POPZOE</span>'
                    : platform === 'POPSUR'
                        ? '<span class="badge" style="background:#22c55e;color:white;">POPSUR</span>'
                        : `<span class="badge" style="background:#6b7280;color:white;">${platform || 'UNKNOWN'}</span>`;
...
function initializePage(pageId) {
    switch (pageId) {
        ...
        case 'historical': initHistoricalPage(); break;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const historicalRefreshBtn = document.getElementById('historicalRefreshBtn');
    if (historicalRefreshBtn) {
        historicalRefreshBtn.addEventListener('click', () => initHistoricalPage(true));
    }
});
